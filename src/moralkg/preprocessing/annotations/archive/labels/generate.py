...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Skip papers that already have labels files generated",
    )
    parser.add_argument(
        "--skip-existing-arguments",
        action="store_true",
        help="Skip papers that already have arguments files generated",
    )
    parser.add_argument(
        "--overwrite-labels",
        action="store_true",
        help="Overwrite labels files (default: append)",
    )
    parser.add_argument(
        "--overwrite-arguments",
        action="store_true",
        help="Overwrite arguments files (default: append)",
    )
    parser.add_argument(
        "--append-labels",
        action="store_true",
        help="Append to labels files (default behavior)",
    )
    parser.add_argument(
        "--append-arguments",
        action="store_true",
        help="Append to arguments files (default behavior)",
    )
    parser.add_argument(
    ...
    for i, identifier in enumerate(identifiers, 1):
        ...
        # Determine skip behavior based on granular flags
        skip_due_to_existing = False
        
        if args.skip_existing and args_path.exists() and labels_path.exists():
            skip_due_to_existing = True
            print(f"Skipping {identifier}: both output files already exist")
        elif args.skip_existing_labels and labels_path.exists():
            skip_due_to_existing = True
            print(f"Skipping {identifier}: labels file already exists")
        elif args.skip_existing_arguments and args_path.exists():
            skip_due_to_existing = True
            print(f"Skipping {identifier}: arguments file already exists")
        ...
        # Save results
        # Determine overwrite behavior for arguments
        overwrite_args = (
            args.overwrite_arguments or 
            (args.overwrite and not args.append_arguments)
        )
        
        # Determine overwrite behavior for labels  
        overwrite_labels = (
            args.overwrite_labels or 
            (args.overwrite and not args.append_labels)
        )
