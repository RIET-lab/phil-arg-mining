#!/usr/bin/env python3
"""
Simple TF-IDF Vectorizer with K-means Clustering on title column only of Phil Papers Dataset.
"""
...
def main():
    ...
    # Set up logging
    logger = setup_logging(output_dir, 'tfidf_title')
    ...
    # Work with title column only
    df_title = df[['title']].dropna().copy()
    logger.info(f"Working with title column only: {len(df_title)} records")
    
    # Filter out empty titles
    df_title: pd.DataFrame = df_title.loc[df_title['title'].str.len() > 0].copy()
    logger.info(f"After filtering: {len(df_title)} records")
    
    # Store original data for evaluation
    original_data = df_title.copy()
    
    # TF-IDF vectorization (directly on title column)
    with time_operation(logger, "TF-IDF vectorization"):
        ...
        tfidf_matrix = vectorizer.fit_transform(df_title['title'])
    ...
    with time_operation(logger, "t-SNE computation with Barnes-Hut approximation"):
        tsne = TSNE(
            ...
            random_state=42,
            method='barnes_hut', 
            perplexity=min(30, len(df_title)//4), 
            verbose=1 if len(df_title) > 10000 else 0 
        )
    ...
    save_npz(output_dir / "tfidf_matrix_title.npz", tfidf_matrix)
    
    # Save vectorizer for feature names
    with open(output_dir / "tfidf_vectorizer_title.pkl", "wb") as f:
        pickle.dump(vectorizer, f)
    
    # Save KMeans model
    with open(output_dir / "kmeans_model_title.pkl", "wb") as f:
        pickle.dump(kmeans, f)
    
    # Save full dataset with cluster assignments
    df_title.to_csv(output_dir / "tfidf_clusters_title.csv", index=False)
    ...
    for i, cluster_id in enumerate(sorted(np.unique(clusters))):
        cluster_data = df_title.loc[df_title['cluster'] == cluster_id].copy()
    ...
    sample_output = output_dir / 'tfidf_title_n100.csv'
    ...
    try:
        sample_indices = get_sample_indices(final_sample, df_title)
        
        # Create enhanced t-SNE plot with samples highlighted
        create_enhanced_mds_plot(
            mds_coords, clusters, sample_indices,
            output_dir / 'tfidf_title_tsne_with_samples.png',
            'TF-IDF K-means (Title Only) - t-SNE Visualization', logger
        )
        ...
        plt.title('TF-IDF K-means Clustering - t-SNE Visualization (Title Only)')
        plt.xlabel('t-SNE Dimension 1')
        plt.ylabel('t-SNE Dimension 2')
        plt.savefig(output_dir / 'tfidf_title_tsne_basic.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    # Save evaluation summary
    save_evaluation_summary(clustering_metrics, representativeness_metrics, output_dir, 'tfidf_title', logger)
