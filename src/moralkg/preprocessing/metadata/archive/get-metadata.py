...
# Use get-metadata.sh to run this script with the default arguments for PhilPapers.

# This script was taken from The-Pile-PhilPapers repository:
...
def getFile(serverString, command, verbose=1, sleepTime=0):
    ...
    if r.status_code != 200:
        ...
        if nRecoveries > maxRecoveries:
            print("Too many recoveries, giving up")
            return None
        print("Retrying in 5 seconds...")
        time.sleep(5)
        return getFile(serverString, command, verbose, sleepTime + 5)
